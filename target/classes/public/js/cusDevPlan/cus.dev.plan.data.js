layui.use(['table','layer'],function(){
    ...
    table.on('toolbar(cusDevPlans)', function (data) {
        if (data.event == "add") { // 添加计划项
            // 打开添加或修改计划项的页面
            openAddOrUpdateCusDevPlanDialog(null);
    ...
    table.on('tool(cusDevPlans)', function (data) {
        if (data.event == "edit"){   // 编辑按钮
            // 打开添加或者修改计划项的页面
            openAddOrUpdateCusDevPlanDialog(data.data.id);
        } else if (data.event == "del"){
            // 根据传入的用户id进行删除对应的用户信息
            deleteCusDevPlanDialog(data.data.id);
        }
    });
     ...
     * 删除计划项的信息
     */
    function deleteCusDevPlanDialog(id) {
    ...
    function openAddOrUpdateCusDevPlanDialog(id) {
        var title = "客户开发计划-添加";
        // ?sId="+$("[name='id']").val()：目的就是将用户的id传递到添加页面的request域中，从而执行给谁进行添加数据
    ...
    function updateSaleChanceDevResult(devResult) {
        ...
        var id = $("[name='id']").val();
        // 当前更新的是营销机会数据的开发状态
        layer.confirm("你确定要更新开发状态吗?",{icon:3}, function (index) {
            $.post(ctx + "/sale_chance/updateSaleChanceDevResult", {id: id, devResult: devResult}, function (result) {
                if (result.code == 200) {
